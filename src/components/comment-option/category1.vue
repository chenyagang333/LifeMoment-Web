...
const iconStype = computed(() => { // 计算icon大小
    return { fontSize: props.size + 'px',marginBottom:'-'+props.bottom }
})
const countStype = computed(() => { // 计算count大小
...
if (keyExists) {
    if (props.isActive) { // 如果是激活状态，则icon初始样式为激活的样式
...
const countComputed = computed(() => {
   return props.count! + (activeRef.value ? (props.isActive ? 0 : 1) : (props.isActive ? -1 : 0))
});
